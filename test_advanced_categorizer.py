#!/usr/bin/env python3
"""
Test script for advanced expense categorizer to verify >90% accuracy
"""
import sys
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '.'))

from backend.ml_models.advanced_expense_categorizer import AdvancedExpenseCategorizer

def test_advanced_categorizer():
    """
    Test the advanced categorizer to ensure it meets the 90% accuracy requirement
    """
    print("[TEST] Testing Advanced Expense Categorizer")
    print("=" * 50)

    # Create and train the advanced categorizer
    categorizer = AdvancedExpenseCategorizer()

    print("[TRAIN] Training the advanced model...")
    accuracy = categorizer.train()

    print(f"[DONE] Training completed with accuracy: {accuracy:.1%}")
    ...
    print(f"\n[TEST] Testing {len(test_cases)} comprehensive cases...")
    ...
    if overall_accuracy >= 0.90:
        print(f"[SUCCESS] Advanced categorizer achieved {overall_accuracy:.1%} accuracy (>90%)!")
    else:
        print(f"[WARNING] Advanced categorizer achieved {overall_accuracy:.1%} accuracy (<90%)")
...
if __name__ == "__main__":
    success = test_advanced_categorizer()

    if success:
        print(f"\n[COMPLETE] All tests passed! Advanced categorizer is ready with >90% accuracy.")
        sys.exit(0)
    else:
        print(f"\n[ERROR] Some tests failed. Accuracy requirement not met.")
        sys.exit(1)